...
`commands/clean.ts` MUST scope tag deletion to the active spec. The specId resolution MUST use the same deterministic order as `recoverState()`:
1. Read `specId` from `state.json` (via `loadState()`, which already calls `recoverState()` when state is missing).
2. If `state.specId` is non-null after loading, delete only `steel/<specId>/*-complete` tags.
3. Only if `state.specId` is null after the full recovery chain (state.json → branch detection → specs directory fallback) should the command fall back to deleting all `steel/*/*-complete` tags and warn the user that it cannot determine the active spec.
