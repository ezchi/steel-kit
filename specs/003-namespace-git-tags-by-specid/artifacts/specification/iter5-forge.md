...
**Deliberate divergence — spec-file scanning scope**: The doctor's spec-file check (whether any spec directory contains stage artifacts) intentionally remains broad — scanning all entries under `specs/`, not scoped to a single specId. This is a pre-existing behavior that predates this feature. The doctor is a coarse diagnostic ("is there any recoverable evidence?"), not a precise recovery engine. Narrowing the spec-file check to match `recoverState()`'s specId scoping is a valid future improvement but is out of scope for this tag-namespacing change, which only modifies the tag pattern. The tag check IS scoped to specId (steps 1–4 above); the file check remains broad as-is.
