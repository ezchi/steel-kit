...
**In `src/workflow.test.ts`** (tag creation and recovery):
- Tag creation produces `steel/<specId>/<stage>-complete` (AC-1)
- Multi-spec tag coexistence: create tags for specId A and specId B, verify both sets persist (AC-2)
- Recovery with known specId finds namespaced tags (AC-4)
- Recovery with null specId skips tag-based recovery (AC-5)
- `resolveSpecId()` with specs-dir fallback: branch detection fails, specs/ directory has entries → returns highest-numbered entry
- `resolveSpecId()` with branch detection: returns specId from branch prefix
- Legacy flat tags present alongside namespaced → recovery ignores legacy, processes namespaced (AC-9 — recovery path)

**In `src/doctor.test.ts`** (doctor tag detection):
- Doctor with branch-derived specId uses scoped `steel/<specId>/*-complete` pattern (AC-11)
- Doctor with specs-dir fallback: branch fails, specs/ exists → uses highest-numbered entry for scoped pattern
- Doctor with no resolvable specId uses broad `steel/*/*-complete` pattern (AC-12)
- Doctor detects namespaced tags as recoverable when specId known (AC-6)
- Legacy flat tags present alongside namespaced → doctor ignores legacy (AC-9 — doctor path)

**In `commands/clean.test.ts`** (new file — scoped cleanup):
- Clean with known specId only deletes that spec's tags, other spec's remain (AC-3)
- Clean with null specId from state.json, branch resolves specId → scoped deletion (AC-13 — branch path)
- Clean with null specId from state.json, branch fails, specs-dir resolves specId → scoped deletion (AC-13 — specs-dir path)
- Clean with unresolvable specId falls back to `steel/*/*-complete` with warning
- Legacy flat tags present alongside namespaced → clean ignores legacy (AC-9 — clean path)

**Files to modify**:
- `src/workflow.test.ts` — tag creation, recovery, and resolveSpecId tests
