...
- When stale files are found and deleted, a log message reports the count (e.g., `"Removed N stale .gemini/commands/ TOML files"`)
...
### Steps:
1. In `commands/init.test.ts`: add test that init cleans up stale `.gemini/commands/steel-*.toml` files and logs the cleanup count
...
### Done when:
- Init cleanup test: pre-seeded `.gemini/commands/steel-*.toml` files are deleted after init, and cleanup count is logged
