...
**Responsibility**: Type definitions, preset registry, resolution logic, and validation for git branching configuration.
...
**Dependencies**: `src/config.ts` (for `SteelConfig` type)

**Key design decisions**:
- Separate file from `config.ts` to keep config loading and git-specific resolution as distinct concerns.
...
- Add `STEEL_GIT_*` environment variable handling to the env override section (FR-7).
...
**What stays the same**: `loadConfig()`, `saveConfig()`, `getSteelDir()`, `getSpecsDir()`, `getSpecDir()`, `getConfigPath()` — all untouched.
...
// New types in src/git-config.ts
...
**Internal dependencies (new modules)**:
- `src/git-config.ts` depends on `src/config.ts` (for `SteelConfig` type)
- `src/spec-id.ts` depends on `src/git-config.ts` (for `validateSpecIdComponent()`)
...
7. **Update `src/cli.ts`**: Add `--id` option to `specify` command definition.
8. **Write integration tests** for `initBranch()` with various configs (AC-6–8, AC-16–17).
...
14. **Write tests** for init config merging (AC-13, AC-27–28).

**Verification gate**: Full test suite passes. `steel init` on fresh project prompts for git config.

### Phase 5: Final verification

15. **End-to-end smoke test**: Run through a complete `steel specify` → branch creation → recovery → doctor flow with each preset (`steel`, `github-flow`, `gitflow`) and with `--id`.
16. **AC-29**: `npm run build && npm test && npm run lint` all pass clean.
...
**Mitigation**: The two-phase approach (merge all sources → resolve presets) has a single, testable code path. AC-14 and AC-15 verify env var behavior explicitly.
...
- `resolveGitConfig()` with env var overrides (AC-14)
- `resolveGitConfig()` with invalid workflow env var → warning, default applied (AC-15)
...
- `initBranch()` with `feature/` prefix creates correct branch (AC-6)
- `initBranch()` with `eda-` prefix and `--id` creates correct branch (AC-7)
- `initBranch()` with default config creates `spec/` branch (AC-8)
