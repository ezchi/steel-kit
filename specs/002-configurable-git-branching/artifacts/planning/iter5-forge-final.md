...
- **Three-tier validation** (addresses NFR-5 fully):
  ...
  3. `validateComposedRef()` — validates the full composed ref `branchPrefix + specId` as a valid git ref. Called by `resolveGitConfig()` with a dummy suffix (e.g. `000-test`) to catch prefixes that produce invalid composed refs. Also called by `initBranch()` with the real specId as a defensive check.
- `resolveGitConfig()` is the **primary enforcement point** for config validation (NFR-5). It validates all fields and runs the composed-ref smoke test. This ensures all consumers (specify, recovery, doctor) get validated config.
- Validation functions exported so both `resolveGitConfig()` (NFR-5) and `steel init` (FR-26) share the same logic (CLR-6).
- Preset values (`spec/`, `feature/`) skip per-field validation since they are known-valid (NFR-5). Composed-ref smoke test still runs for non-preset prefixes.
...
**Also changed**: `initConfig()` — add `deferWrite` option so it can return provider selections without eagerly writing to disk (required for re-init config preservation in FR-27).
...
- **Legacy compatibility case**: When the configured prefix is not `spec/` but the current branch starts with `spec/<specId>`, `checkDrift()` shall treat this as a recognized compatibility case:
  1. **Suppress all branch-prefix mismatch diagnostics** — both `drift-specid-branch` (current branch vs expected branch) and `drift-state-branch` (`state.branch` vs expected branch). In the legacy case, `state.branch` will be `spec/<specId>` while the expected branch would be `feature/<specId>` — both mismatches are expected and must not produce failures.
  2. **Emit only** a single `warn`-severity diagnostic (`drift-legacy-prefix`) with remediation suggesting the user rename the branch to match the configured prefix or update the config to use `spec/`.
  3. The specId is still extracted correctly from the `spec/` prefix for directory-existence checks.
  This ensures AC-12 produces a warning only, with no accompanying failures.

**Unchanged**: All other check functions, severity matrix (add `drift-legacy-prefix: 'warn'` entry), `runDoctor()` orchestration.
...
- **Remove the blanket `catch` around `initBranch()`** (current lines 40-45). The existing code swallows all branch-creation errors and continues on the wrong branch. This must be replaced with fail-fast behavior: if `initBranch()` throws (dirty tree per FR-11, missing baseBranch per FR-12, or any git error), the command must abort immediately with the error message. No state or artifacts should be written after a branch-creation failure.
...
- **Re-init config preservation** (FR-27, AC-27): The current `initConfig()` constructs a fresh normalized config and eagerly writes it to disk (line 128-137 in `config.ts`), discarding any existing unknown keys in the process. A post-write raw-merge patch cannot restore already-discarded fields. To satisfy FR-27, the eager-write flow must be replaced:
  1. **Refactor `initConfig()`** to accept an option (e.g. `{ deferWrite: true }`) that causes it to return the selected provider values without writing to disk. This preserves the existing `skipWrite` behavior for callers that need it.
  2. In `cmdInit()`, after collecting all user inputs (providers + git config):
     a. If `.steel/config.json` exists, read it as raw JSON via `JSON.parse(fs.readFileSync(...))`.
     b. Merge provider selections (`forge`, `gauge`) and git config (`git`) onto the raw JSON object, preserving all other existing keys.
     c. If no existing file, start from `DEFAULT_CONFIG` and add provider + git selections.
     d. Write the final merged object via a single `fs.writeFileSync(...)`.
  3. This ensures unknown top-level keys (including future ones) are preserved during re-init, and respects the user's overwrite decision in a single write, not a write-then-patch sequence.
...
6. Composed-ref smoke test: validate branchPrefix + a known-valid dummy suffix
   (e.g. "000-test") via validateComposedRef() to catch prefixes that pass
   per-field checks but produce invalid composed refs. This ensures
   resolveGitConfig() is the primary enforcement point per NFR-5.
   initBranch() may re-validate with the real specId defensively, but
   resolution catches invalid config early for all consumers (recovery, doctor).
...
12. **Update `commands/init.ts`**: Add base branch and branch prefix prompts with validation (FR-24–FR-27). Refactor `initConfig()` to support `deferWrite`, then do a single raw JSON read/merge/write in `cmdInit()` to preserve all existing fields during re-init.
...
- `resolveGitConfig()` with non-preset prefix runs composed-ref smoke test during resolution
...
- Invalid `--id` (e.g. `hello world`) → error before any branch/file creation (FR-14)
- Colliding `--id` (existing spec dir) → error before any branch/file creation (FR-15a, AC-30)
- `initBranch()` failure (e.g. dirty tree) → command aborts, no state/artifacts written (FR-11)
...
- Invalid branchPrefix (e.g. `feat..ure/`) re-prompts with clear error, only corrected value persisted (FR-26)
