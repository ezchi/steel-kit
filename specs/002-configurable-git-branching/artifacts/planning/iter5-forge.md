...
- **Re-init config preservation** (FR-27, AC-27): The current `initConfig()` eagerly writes a normalized config before returning, and `mergeConfig()` only preserves known fields. To satisfy FR-27 (preserve all existing fields including future ones), the git config prompts must use a **raw JSON read/merge/write** path that bypasses the lossy `loadConfig()`/`mergeConfig()` pipeline:
  1. After `initConfig()` completes (which handles forge/gauge providers), read `.steel/config.json` as raw JSON via `JSON.parse(fs.readFileSync(...))`.
  2. Set or merge only the `git` sub-object on the raw JSON object.
  3. Write back the full raw JSON via `fs.writeFileSync(...)`.
  This preserves unknown top-level keys that `mergeConfig()` would drop, satisfying AC-27's requirement that re-init preserves `forge`, `gauge`, `maxIterations`, `autoCommit`, `specsDir`, and any future fields.
...
12. **Update `commands/init.ts`**: Add base branch and branch prefix prompts with validation (FR-24–FR-27). Use raw JSON read/merge/write for git config to preserve all existing fields during re-init.
