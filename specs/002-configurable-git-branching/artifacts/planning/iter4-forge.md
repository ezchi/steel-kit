...
- **Legacy compatibility case**: When the configured prefix is not `spec/` but the current branch starts with `spec/<specId>`, `checkDrift()` shall treat this as a recognized compatibility case:
  1. **Suppress** the normal branch-mismatch failure (`drift-specid-branch`) that would fire because `spec/002-test` != `feature/002-test`.
  2. **Emit only** a `warn`-severity diagnostic (`drift-legacy-prefix`) with remediation suggesting the user rename the branch to match the configured prefix or update the config to use `spec/`.
  3. The specId is still extracted correctly from the `spec/` prefix for directory-existence checks.
  This ensures AC-12 produces a warning, not a failure, for legacy branches.
