...
- FR-5: A `resolveGitConfig(config: SteelConfig)` function shall return a `ResolvedGitConfig`:
  ...
  export interface ResolvedGitConfig {
    ...
    branchPrefix: string;
    baseBranch: string;
    developBranch?: string;       // present only when gitflow preset is active
  }
  ```
...
- FR-14: The `specify` command in `src/cli.ts` shall accept an optional `--id <value>` option for supplying a custom spec identifier. The value is an arbitrary string (e.g. `21`, `PROJ-21`, `abc`).
...
- NFR-6: The `--id` flag shall accept any non-empty string value to support arbitrary ticket identifiers.
...
- AC-19: `npm run build` compiles without errors; `npm test` passes; `npm run lint` type-checks.
