...
**Files**: `src/spec-id.ts` (new), `commands/specify.ts` (remove local `generateSpecId`)
...
14 tasks across 5 phases. Tasks within each phase can be parallelized (e.g. T1, T2, T3 in parallel). Each phase has a test task as its gate. Total coverage: all 34 acceptance criteria, all FRs, all NFRs.
