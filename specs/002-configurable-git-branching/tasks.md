...
### Task 1: Add git config types to `src/config.ts` and create `src/git-config.ts`
**Files**: `src/config.ts` (add types), `src/git-config.ts` (new)
**Dependencies**: None
**Acceptance Criteria**: AC-1, AC-2, AC-3, AC-18, AC-24, AC-25, AC-26

**Subtasks**:
0. In `src/config.ts`: add `GitWorkflow`, `GitConfig`, `ResolvedGitConfig` type exports and `git?: GitConfig` field to `SteelConfig` (FR-1, FR-2, FR-3, FR-5). This enables `git-config.ts` to import types.
...
### Task 3: Update `src/config.ts` — deep-merge, env vars, deferWrite
**Files**: `src/config.ts`
**Dependencies**: Task 1 (types already added in Task 1)
**Acceptance Criteria**: AC-14, AC-15

**Subtasks**:
1. Update `mergeConfig()` to deep-merge `git` sub-object: `git: { ...base.git, ...(override.git ?? {}) }` (FR-6)
2. Add `STEEL_GIT_*` env var handling in `loadConfig()` after YAML/JSON merge (FR-7):
...
1. `commands/init.test.ts`:
   - Init prompts with correct defaults: baseBranch=`main`, branchPrefix=`spec/` (AC-13)
   - Init stores user-supplied git values under `git` key (AC-13)
   - Re-init preserves existing `forge`, `gauge`, `maxIterations`, `autoCommit`, `specsDir` (AC-27)
   - Re-init preserves unknown top-level fields (seed config with extra key like `futureField: true`, verify preserved after re-init) (FR-27 forward-compat)
...
Phase 1:  [T1] ─┬─► [T2] ─┐
                └─► [T3] ─┤──► [T4] (gate)
                           │
                    ┌──────┘ (T4 must pass before Phase 2)
                    ▼
Phase 2:        [T5] ──► [T6] ──► [T7] (gate)
                                    │
                    ┌───────────────┘ (T7 must pass before Phase 3)
                    ▼
Phase 3:  [T8] ────────────┐
          [T9] ────────────┤──► [T10] (gate)
                                  │
                    ┌─────────────┘ (T10 must pass before Phase 4)
                    ▼
Phase 4:  [T11] ───────────┐
          [T12] ───────────┤──► [T13] (gate)
                                  │
                    ┌─────────────┘ (T13 must pass before Phase 5)
                    ▼
Phase 5:  [T14] (final verification)

Within-phase parallelism: T2∥T3, T8∥T9, T11∥T12
Strictly sequential: T1→T2, T1→T3, T5→T6→T7
Cross-phase gates: T4→T5, T7→T8/T9, T10→T11/T12, T13→T14
```

## Summary

14 tasks across 5 phases. Phase 1: T1 first (adds types + git-config), then T2 and T3 in parallel (both depend on T1), then T4 (tests). Phase 2: T5→T6→T7 (sequential — T6 depends on T5). Phase 3: T8∥T9 then T10. Phase 4: T11∥T12 then T13. Phase 5: T14 (final verification). Each phase has a test task as its gate. Total coverage: all 34 acceptance criteria, all FRs, all NFRs.
