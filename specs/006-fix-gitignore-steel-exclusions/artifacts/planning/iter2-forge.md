...
# Check that non-steel .claude/commands/ files are still tracked
non_steel_commands=$(git ls-files .claude/commands/ | grep -v steel-)
non_steel_skills=$(git ls-files .agents/skills/ | grep -v steel-)
# These may be empty if no non-steel files exist, which is fine.
# If they exist, they must still be tracked.
...
- **Risk:** Non-steel files affected. **Mitigation:** Patterns are specific (`steel-*` prefix, `.steel/*` with negations). AC-4 verification confirms.
