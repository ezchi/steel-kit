...
4. **Git tags are not consulted.** This test does not need an actual git repo because `cmdStateReset` shells out nowhere — verify by inspection that the function body in 2a only calls `createInitialState()` + `saveState()`. (Optional fixture if desired: `git init` the temp dir, `git tag steel/001-foo/specification-complete`, run reset, assert `git tag --list` still includes it. Adds complexity for low value — keep optional.)
...
2. **`describe('FR-5 decline detection (composition)')`** — covers AC-6 / NFR-5 FR-5 row. **This is the test missing from iter 1 (gauge B-3).**
   ...
   2. Mock `confirm` from `src/utils.js` to return `false` (matches the pattern used in `commands/clean.test.ts` for declined-confirmation tests — use `vi.mock('../src/utils.js', ...)` at module-load time, OR `vi.spyOn` if utils exports `confirm` as a mockable named export).
   3. Run `cmdClean()` (the existing exported function from `commands/clean.ts`).
...
### 5d. FR-8 propagation smoke (optional, scope-aware)

`src/command-installer.test.ts` currently tests `renderAgentSkill` (a pure-string function) — not `installProjectCommands` end-to-end. Two scope choices:

- **Option A (recommended, low cost):** add a single assertion to existing `renderAgentSkill` tests checking the FR-3 prompt string survives the agent-skill MD adaptation. One-line `expect(rendered).toContain('A previous workflow')`.
- **Option B (higher cost, fuller coverage):** add a temp-dir fixture that runs `installProjectCommands(tempDir)` and reads `.claude/commands/steel-specify.md` to assert FR-3 string presence. Requires a temp dir, a fake config, and a teardown step.

Choose **Option A** for this spec — the propagation mechanism (`adaptMarkdownForAgentSkill`) is the only Steel-Kit-specific transformation; if the source contains the string and the adapter doesn't strip it, the installed copy will contain it. Implement-stage agents may upgrade to Option B if Option A's coverage is judged insufficient at code review.
